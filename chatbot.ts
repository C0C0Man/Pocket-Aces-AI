...
class PokerActionProvider extends ActionProvider<WalletProvider> {
  ...
  @CreateAction({
    name: "fold",
    description:
      "Folding in Texas Hold'em means forfeiting your hand and exiting the current round of betting. Once you fold, you are no longer eligible to win the pot, and you do not have to contribute any more chips to the betting round. However, you must wait for the next hand to participate again.",
  ...
  @CreateAction({
    name: "call",
    description:
      "Calling in Texas Hold'em means matching the current bet in order to stay in the hand. Unlike raising (which increases the bet) or folding (which forfeits the hand), calling simply keeps you in the game without escalating the stakes. Calling can be a strong strategic move or a costly mistake, depending on the situation. A well-timed call can set up a powerful trap for your opponents, allow you to see additional cards at a good price, or keep your range balanced. However, excessive calling (often referred to as being a calling station) can lead to losses if done without a clear reason.",
    schema: MyActionSchema,
  })
  async callAction(
    walletProvider: CdpWalletProvider,
  ...
  @CreateAction({
    name: "getHighestBet",
    description:
      "Get the highest bet placed in your game so you can make informed decisions on how you would like to bet",
    schema: MyActionSchema,
  })
  async getHighestBet(
    walletProvider: CdpWalletProvider,
  ...
  @CreateAction({
    name: "getAIBalances",
    description:
      "Get the balances of all players so you can make a more informed decision on if you want to call, raise, or fold.",
    schema: MyActionSchema,
  })
  async getAIBalances(
    walletProvider: CdpWalletProvider,
...
async function initializeAgent() {
  try {
    ...
    const config = {
      ...
      networkId: "base-sepolia", // Hardcoded to ensure correct network
    };
